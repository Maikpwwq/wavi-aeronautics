...
describe('priceUtils - Unit & Property Tests', () => {
  ...
  describe('calculateCopPrice', () => {
    ...
    it('calculates COP price correctly for numeric USD values', () => {
      // Math: (100 + 30) * 1.5 * 4000 = 130 * 6000 = 780,000 COP
      const result = calculateCopPrice(100)
      expect(result).toContain('780')
    })
  ...
  describe('parseCopCurrency', () => {
    ...
    it('Property-Based Test: Parsing formatted currency yields non-negative integer', () => {
      fc.assert(
  ...
  describe('parseProductPrices', () => {
    it('updates new migrated products with price in USD', () => {
      ...
      expect(products[0].precio).toContain('780')
    })

    it('updates legacy products with precio', () => {
