...
describe("FeaturedBrands Component Tests", () => {
  it("renders section title, badge, and descriptive subtitle", () => {
    ...
    expect(
      screen.getByRole("heading", { level: 2, name: /Marcas Destacadas/i }),
    ).toBeInTheDocument();
    expect(
      screen.getByText(/PARTNERS OFICIALES & HARDWARE DE ÉLITE/i),
    ).toBeInTheDocument();
    expect(
      screen.getByText(
        /Distribución y venta de componentes de alta gama para pilotos FPV/i,
      ),
    ).toBeInTheDocument();
  });
  ...
  it("renders all 8 featured brands dynamically from configuration", () => {
    ...
    FEATURED_BRANDS.forEach((brand) => {
      ...
      const image = screen.getByAltText(
        new RegExp(`Logo de ${brand.name}`, "i"),
      );
  ...
  it("supports custom brands prop if provided", () => {
    ...
    expect(
      screen.getByRole("link", {
        name: /Ver productos de la marca DJI Enterprise/i,
      }),
    ).toHaveAttribute("href", "/tienda?marca=dji");
    expect(
      screen.getByRole("link", {
        name: /Ver productos de la marca Caddx FPGA/i,
      }),
    ).toHaveAttribute("href", "/tienda?marca=caddx");
    expect(
      screen.queryByRole("link", {
        name: /Ver productos de la marca BetaFPV/i,
      }),
